...
export default function Menu({ navigation, route }) {
  ...
  useEffect(() => {
    async function getHospitals() {
      await api.get("/hospitals").then((response) => {
  ...
  const positionUser = allUsers.map((userObject, index) => {
    if (userObject.user_id_pk == user.user_id) {
      return index + 1;
    }
  });
        ...
        <HeaderRight>
          <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
            <UserCombo
              sourceImg={
                user.user_pic
                  ? { uri: "data:image/jpeg;base64, " + user.user_pic }
                  : logo
              }
              onpress={() => {
                ...
                <UserCombo
                  ...
                  sourceImg={
                    user.user_profile_pic
                      ? {
                          uri:
                            "data:image/jpeg;base64, " + user.user_profile_pic,
                        }
                      : logo
                  }
                  onpress={() => {
      ...
      <Body>
        ...
        <FlatList
          ...
          renderItem={(post) => {
            ...
            return (
              <TextCard
                title={postOwner ? postOwner.user_name : "Loading..."}
